...
mysqldb = MySQLConnector(app, "registration") # create the connection
	...
	if request.method=="POST":
		...
		if (len(email)<1 or len(password)<1):
			...
			if (len(email)>0):
				...
				print salt_key, "THis is the salt key"
				if (salt_key==False):
		...
		clearSession()	
	if isValid == True:
		return redirect ('/dashboard/'+ str(session['userid']) + '')
	else:	
...
def dashboard(userid):
	if isValidSession(session['login'])!=True:
		return redirect ("/")
	return render_template('dashboard.html', id = userid)
